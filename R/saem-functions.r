...
#' @param config List. Model configuration including design matrices, hyperparameters, and algorithm settings.
#' @param state List. Current state of the algorithm (phi, beta, gamma, alpha, etc.).
#' @param sa_func Function. Stochastic approximation step function (SA-step) to update sufficient statistics.
#' @param m_func Function. M-step function to update model parameters.
#' @param mh_update_func Function. Optional function to update proposal distributions for the Metropolis step.
#' @return Updated \code{state} list after one SAEM iteration.
#' @keywords internal
perform_saem_iteration <- function(k, config, state, sa_func, m_func, mh_update_func) {
...
run_saem_full <- function(config, state, sa_func, m_func, mh_update_func) {
  for (k in 1:config$num_iterations) {
    state <- perform_saem_iteration(k, config, state, sa_func, m_func, mh_update_func)
  }
  return(state)
}
...
#' @param m_func Function. M-step function to update model parameters.
#' @param mh_update_func Function. Metropolis-Hastings update function.
#' @return Final \code{state} list after completing all SAEM iterations with shrinkage for fixed effects.
#' @keywords internal
run_saem_with_fixed_effects <- function(config, state, sa_func, m_func, mh_update_func) {
  # Burn-in iterations
