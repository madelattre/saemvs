# compile_model <- function(g_fun, cpp_path = NULL, so_path = NULL) {
#   if (is.null(cpp_path)) {
#     cpp_path <- tempfile(fileext = ".cpp")
#   }
#   if (is.null(so_path)) {
#     so_path <- sub("\\.cpp$", .Platform$dynlib.ext, cpp_path)
#   }

#   body_txt <- deparse(body(g_fun))
#   body_txt <- paste(body_txt, collapse = "")

#   # Adapter indexation R (1-based) vers C++ (0-based)
...
#   writeLines(code_cpp, cpp_path)

#   # Compiler (crée le .so)
#   Rcpp::sourceCpp(cpp_path, rebuild = TRUE, verbose = FALSE)

#   invisible(list(cpp_path = cpp_path, so_path = so_path))
# }
...
compile_model <- function(g_fun, build_dir = "compiled_models") {
  ...
  # # Noms de fichiers
  # cpp_path <- file.path(build_dir, "model.cpp")
  # so_path  <- file.path(build_dir, paste0("model", .Platform$dynlib.ext))

  # Extraire le corps de la fonction R
  body_txt <- deparse(body(g_fun))
  body_txt <- paste(body_txt, collapse = "")
  # Adapter l'indexation R (1-based) vers C++ (0-based)
  body_txt <- gsub("phi\\[(\\d+)\\]", "phi[\\1-1]", body_txt)
...
', body_txt)
  ...
  # Crée un fichier temporaire pour le code C++
...
#   # Compiler et charger dynamiquement
#   Rcpp::sourceCpp(code = code_cpp, rebuild = TRUE, verbose = FALSE)
# }
