...
#'
#' Internal function to verify that the data provided by the user is consistent
#' with the SAEMVS model specification.
#'
...
#' \itemize{
#'   \item If \code{phi_to_select_idx} is non-empty, \code{x_candidates} must be
#'    provided.
#'   \item \code{x_candidates}, if present, must have one row per sequence in
#'   \code{y_series}.
#'   \item \code{x_forced} and \code{x_forced_support}, if present, must be
#'   numeric matrices with compatible dimensions.
#' }
#'
#' @keywords internal
setGeneric(
...
#'
#' Internal function to verify that the initial values provided by the user
#' match the SAEMVS model and data dimensions.
#'
  ...
  function(init, data, model) {
    ...
    if (!is.null(init@beta_candidates)) {
      if (!is.matrix(init@beta_candidates) ||
        ...
        stop(
          sprintf(
            paste0(
              "'beta_candidates' must be a matrix with %d columns ",
              "(matching intercept); got %d."
            ),
            n_phi,
            ifelse(
              is.null(ncol(init@beta_candidates)),
              NA,
              ncol(init@beta_candidates)
            )
          )
        )
      }
    ...
    if (!is.null(init@beta_forced)) {
      if (!is.matrix(init@beta_forced) || ncol(init@beta_forced) != n_phi) {
        stop(
          sprintf(
            paste0(
              "'beta_forced' must be a matrix with %d columns ",
              "(matching intercept); got %d."
            ),
            n_phi,
            ifelse(
              is.null(ncol(init@beta_forced)),
              NA,
              ncol(init@beta_forced)
            )
          )
        )
      }
    ...
    if (!is_empty_support(model@x_forced_support)) {
      ...
      if (is_empty_matrix(bfs) && isFALSE(init@default)) {
        stop(
          paste0(
            "Non-empty 'x_forced_support' requires a non-empty initialisation ",
            "of 'beta_forced'. Alternatively, set 'init@default = TRUE'."
          )
        )
      }
    ...
    if (!setequal(zero_diag_idx, phi_idx)) {
      stop(
        sprintf(
          paste0(
            "Mismatch between zero diagonals in 'cov_re' (%s) and ",
            "'phi_fixed_idx' (%s)."
          ),
...
#' Performs the following checks:
#' \itemize{
#'   \item \code{inclusion_prob_prior_a} and \code{inclusion_prob_prior_b}
#'   lengths must match the number of phi parameters subject to selection.
...
#'   \item All values in \code{spike_values_grid} must be smaller than
#'   \code{slab_parameter}.
#' }
#'
#' @keywords internal
  ...
  function(hyper, model, tuning) {
    ...
    if (nb_selected > 0) {
      if (!is.null(hyper@inclusion_prob_prior_a) &&
        ...
        stop(
          sprintf(
            paste0(
              "Hyperparameter 'inclusion_prob_prior_a' must have %d ",
              "components; got %d."
            ),
            nb_selected,
      ...
      if (!is.null(hyper@inclusion_prob_prior_b) &&
        ...
        stop(
          sprintf(
            paste0(
              "Hyperparameter 'inclusion_prob_prior_b' must have %d ",
              "components; got %d."
            ),
            nb_selected,
    ...
    if (!is.null(tuning@spike_values_grid) &&
      ...
      if (!all(tuning@spike_values_grid < hyper@slab_parameter)) {
        stop(
          paste0(
            "'spike_values_grid' values must all be smaller than ",
            "'slab_parameter'."
          )
        )
      }
