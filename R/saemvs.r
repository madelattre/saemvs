...
saemvs_one_map_run <- function(data, model, init, tuning_algo, hyperparam) {
  # fullHyperC
  map <- run_saem(data, model, init, tuning_algo, hyperparam)
  q <- length(model@index_select)
  niter <- tuning_algo@niter
  p <- dim(data@v)[2] # et non -1, car les data sont transformées dans run_saem
...
saemvs_one_ebic_run <- function(
  ...
  ll <- loglik(new_data, new_model, tuning_algo, param, ebic_dim)
