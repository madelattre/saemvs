...
#'   \item Selection of appropriate stochastic approximation (SA) and maximization (M) steps
...
#'   Contains initialization values for the algorithm (parameters, variances, etc.).
#' @param tuning_algo An object of class \code{saemvsTuning}.
#'   Defines algorithmic tuning parameters (number of iterations, burn-in, decay rates, etc.).
...
#'
#' @return An updated \code{state} object containing the results of the SAEM iterations,
...
#'   \item \code{"map_part_select_nofixed"}: MAP with partial selection and no fixed parameters.
#'   \item \code{"map_part_select_fixed"}: MAP with partial selection and some fixed parameters.
...
setMethod(
  ...
  function(data, model, init, tuning_algo, hyperparam) {
    ...
    # --- Step 3: Select correct SAEM variant ---
    # Switch depending on case (MAP vs MLE, full vs partial selection, with/without fixed effects)
    switch(case,
      map_full_select = {
        state <- run_saem_full(
